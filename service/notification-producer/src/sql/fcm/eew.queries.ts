...
const fcmEewIR: any = {"usedParamSet":{"items":true},"params":[{"name":"items","required":false,"transform":{"type":"pick_array_spread","keys":[{"name":"region_id","required":false},{"name":"min_jma_intensity","required":false}]},"locs":[{"a":323,"b":328}]}],"statement":"select\n   u.fcm_token, notification_settings.*\nFROM\n  public.devices AS u\nINNER JOIN\n  public.devices_notification_settings AS notification_settings\n    ON u.id = notification_settings.id\nWHERE\n  u.id IN (\n    SELECT id\n    FROM\n      public.devices_eew_settings\n    WHERE\n      (region_id, min_jma_intensity) IN (\n        :items\n      )\n    )\n  AND fcm_token IS NOT NULL"};
