...
class MessageGenerator {
	...
	handleVxse52(telegram: EarthquakeInformation.Latest.PublicVXSE52): Message & {
		...
		const vxse51 = this.vxse51History.find(
			(e) => e.eventId === telegram.eventId,
		);
		this.vxse51History = this.vxse51History.filter(
			(e) => e.eventId !== telegram.eventId,
		...
		if (vxse51) {
			...
			}で地震がありました。(${magnitude} 深さ${depth})\n震度${maxIntName}を${maxIntPrefName}${
				maxIntMoreThan1 ? 'などで' : 'で'
			}観測しています。\n${telegram.body.comments.free ?? ''}`;
			const ttsMessage = `${
				telegram.body.earthquake.hypocenter.name
			}で地震がありました。${magnitude}、深さ${depth}${`最大震度${maxIntName}を${maxIntPrefName}${
				maxIntMoreThan1 ? 'などで' : ''
			}観測しています。`
				.replace('M', 'マグニチュード')
				.replace('.', '点')
	...
	handleVxse53(telegram: EarthquakeInformation.Latest.PublicVXSE53): Message {
		...
		if (isVolcano) {
			...
			body =
				telegram.body.comments.free === undefined
					? `海外で大規模な噴火が発生しました。${telegram.body.earthquake.hypocenter.name}`
					: '不明';
			ttsMessage = '海外噴火情報 ';
			if (Number.isInteger(serialNo) && serialNo > 1) {
				ttsMessage += `第${serialNo}報`;
	...
	handleVxse62(telegram: EarthquakeInformation.Latest.PublicVXSE62): Message {
		...
		const title = `長周期地震動の観測情報 最大階級 ${telegram.body.intensity?.maxLgInt}`;
	...
	handleEew(
		...
		if (telegram.infoType === '取消') {
			const lastEew = this.eewHistory.find(
				(e) => e.eventId === telegram.eventId,
			);
			const topics = ['eew_all'];
		...
		if (message === '') {
			message += `${body.earthquake.hypocenter.name}で地震${
		...
		if (body.earthquake.condition === '仮定震源要素') {
			...
			if (body.earthquake.hypocenter.depth.value === '0') {
				...
				body.earthquake.condition === '仮定震源要素' ||
				isOnePointEew
			) {
				message += '不明';
			} else {
		...
		// 通常送信
		const topics = isOnePointEew ? ['eew_low_accuracy'] : ['eew', 'eew_all'];
...
String.prototype.toHalfWidth = function () {
	return this.replace(/[！-～]/g, (input) =>
		String.fromCharCode(input.charCodeAt(0) - 0xfee0),
	);
};
